...
special_users = {
    ...
    1669299995:"Ravina Mam"
      # Example: Replace with actual ID and name
...
async def google_pdf_search(query, num_results=10, retries=3, backoff_factor=1):
...
async def handle_message(event):
    ...
    try:
        results = await google_pdf_search(query)
        if results:
            buttons = []
            for title, url in results:
                url_hash = hashlib.md5(url.encode()).hexdigest()
                url_cache[event.chat_id][url_hash] = {'title': title, 'url': url}
                buttons.append([Button.inline(text=f"View PDF: {title[:30]}...", data=f"pdf:{url_hash}")])
            await event.respond('Here are some PDFs I found:', buttons=buttons)
        else:
...
async def start(event):
    ...
    if user_id in special_users:
        ...
        await event.respond(
            ...
            "You can perform up to 2 searches every 2 hours.\n"
            "Just type the title or topic you're looking for, and I'll search for it.\n"
            "Use /help if you need more instructions.\n"
            "Developed by @redmoon0x(Deviprasad Shetty)"
        )
...
async def handle_callback_query(call):
    try:
        ...
        if pdf_info:
            ...
            try:
                await client.send_message(log_channel_id, f"User {call.sender_id} received: {url}")
                await asyncio.sleep(2)
            except ChatIdInvalidError:
